...
class UserInput:
    ...
    def __init__(self) -> None:
        ...
        self.bestbuy['settings']['search'] = self._search_selection
    ...
    def _search_selection(self, *args) -> int:
        ...
        if sku_id:
            selection *= bool(
                self.bestbuy['search by']['sku id']['ids'].get()
            )
        if prices:
            selection *= bool(
                self.bestbuy['search by']['price range']['min'].get()
            )
            selection *= bool(
                self.bestbuy['search by']['price range']['max'].get()
            )
        if price_per_id:
            selection *= bool(
                self.bestbuy['search by']['price per sku id']['ids'].get()
            )
            selection *= bool(
                self.bestbuy['search by']['price per sku id']['prices'].get()
            )
        return selection
