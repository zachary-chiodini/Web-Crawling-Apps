...
class App:
    ...
    def __init__(self, main_window: tk.Tk):
        ...
        self.stop_btn_text = tk.StringVar()
        ...
        self.stop_btn_text.set('Terminate')
        self.stop_btn = tk.Button(
            start_stop, textvariable=self.stop_btn_text,
            state='disable', command=self.stop)
    ...
    def website_selection(self, frame: tk.Frame, row, col, padx, pady) -> None:
        tk.Label(frame, text='Select Website')\
    ...
    def log_display(self, frame: tk.Frame, row, col, padx, pady) -> None:
        ...
        self.log_box.grid(row=row, column=col, padx=padx, pady=(0, pady))
    ...
    @staticmethod
    def address_form(frame: tk.Frame, form_type: Dict,
        ...
        tk.Label(frame, text='Street Address')\
            .grid(row=2, column=col, sticky='w',
                  padx=padx, pady=pady)
        tk.Entry(frame, textvariable=form_type[key]['street1'])\
            .grid(row=3, column=col, sticky='w',
                  padx=padx, pady=pady)
        tk.Entry(frame, textvariable=form_type[key]['street2'])\
        ...
        tk.Label(frame, text='State')\
            .grid(row=7, column=col, sticky='w',
                  padx=padx, pady=pady)
        tk.Entry(frame, textvariable=form_type[key]['state'])\
            .grid(row=8, column=col, sticky='w',
                  padx=padx, pady=pady)
        tk.Label(frame, text='Zip Code')\
            .grid(row=9, column=col, sticky='w',
                  padx=padx, pady=pady)
        tk.Entry(frame, textvariable=form_type[key]['zip code'])\
            .grid(row=10, column=col, sticky='w',
                  padx=padx, pady=pady)
        return None

    @staticmethod
    def set_character_limit(var, limit) -> None:
    ...
    @staticmethod
    def set_force_int(var, commas=False) -> None:
    ...
    def security_code(self, frame: tk.Frame, form_type: Dict,
        ...
        tk.Label(frame, text='CC Security Code')\
            .grid(row=row, column=col, padx=padx, pady=pady)
        ...
        code_wdgt.grid(row=row + 1, column=col, padx=padx, pady=pady)
    ...
    @staticmethod
    def form_trace_remove(form_type: Dict) -> None:
    ...
    @staticmethod
    def button_state(form_type: Dict, button: tk.Button, *args) -> None:
        for key in form_type:
            for value in form_type[key].values():
                if isinstance(value, (tk.IntVar, tk.StringVar)):
                    if not value.get():
    ...
    def ok_or_cancel_buttons(
        ...
        frame.grid(row=row, column=1, columnspan=colspan, sticky='e')
        ...
        cool_glasses.grid(row=row, column=1, padx=padx, pady=pady)
        self.button_state(form_type, cool_glasses)
        for key in form_type:
            for value in form_type[key].values():
                if isinstance(value, (tk.IntVar, tk.StringVar)):
                    ...
                    for nested_value in value.values():
                        nested_value\
                            .trace_add('write', form_trace_callback)
        tk.Button(
    ...
    @staticmethod
    def display_default_text(var, default_text, widget, secure) -> None:
    ...
    def model_selection(self, frame: tk.Frame, row, col, padx, pady) -> None:
        ...
        tk.Label(frame, text='$')\
            .grid(row=row + 2, column=col + 1,
                  padx=(padx, 0), pady=pady)
        min_var = self.form.bestbuy['search by']['price range']['min']
        min_wdgt = tk.Entry(frame, width=6)
        min_wdgt.grid(row=row + 2, column=col + 2,
                      padx=0, pady=pady)
        tk.Label(frame, text='-')\
            .grid(row=row + 2, column=col + 3, padx=0, pady=pady)
        tk.Label(frame, text='$')\
            .grid(row=row + 2, column=col + 4, padx=0, pady=pady)
        max_var = self.form.bestbuy['search by']['price range']['max']
        ...
        if pps_checked.get():
            self.display_default_text(
                pps_ids_var, 'Comma-separated SKU IDs',
                pps_ids_wdgt, False
                )
            self.display_default_text(
                pps_prc_var, 'Comma-separated prices',
                pps_prc_wdgt, False
            )
        else:
    ...
    def page_selection(self, frame: tk.Frame, form_type: Dict,
                       row, col, padx, pady) -> None:
        tk.Label(frame, text='Crawl Pages')\
            .grid(row=row, column=col, columnspan=3, sticky='w',
                  padx=padx, pady=pady)
        ...
        tk.Label(frame, text='-')\
            .grid(row=row + 1, column=col + 1, padx=0, pady=pady)
    ...
    def open_bestbuy_form(self) -> None:
        ...
        tk.Label(
            subframe,
            text='|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|'
            ).grid(row=0, column=4, rowspan=5, padx=10, pady=10)
        self.model_selection(subframe, 0, 5, 10, 10)
        tk.Label(
            subframe,
            text='|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|\n|'
            ).grid(row=0, column=11, rowspan=5, padx=10, pady=10)
    ...
    def start_bestbuy_crawler(self) -> None:
        try:
            ...
            if not self.started:
                self.bestbuycrawler.close()
                return None
            page_start = int(self.form.bestbuy['settings']
                             ['page start'].get())
            page_stop = int(self.form.bestbuy['settings']
                            ['page stop'].get())
            while self.started:
    ...
    def start(self) -> None:
        ...
        if self.form.bestbuy['filled out']['status'].get() == 'complete':
            self.threads = Thread(target=self.start_bestbuy_crawler)
            self.threads.start()
        return None
